...
Le istanze EC2, come qualsiasi server, devono poter accedere a CPU, memoria, rete e soprattutto spazio di archiviazione. L’archiviazione serve per salvare dati, file, [[2.4 Database|database]] o configurazioni che le applicazioni utilizzano durante l’esecuzione. In AWS, questo spazio può essere fornito da diversi tipi di dischi virtuali, a seconda delle esigenze di prestazioni e persistenza.

L’**archiviazione a livello di blocco** memorizza i dati in blocchi separati, come avviene su un disco rigido. Quando un file viene modificato, vengono aggiornati solo i blocchi interessati e non l’intero file. Questo la rende molto efficiente per applicazioni che gestiscono grandi volumi di dati, come [[2.4 Database|database]] o [[3.2 File System|file system]] aziendali, dove le modifiche sono frequenti e localizzate.
...
L’archiviazione dell’Instance Store è volatile: tutti i dati vengono persi quando l’istanza EC2 viene interrotta o terminata. Questo rende rischioso usarla per dati critici come [[2.4 Database|database]] o configurazioni permanenti, poiché non persisterebbero al riavvio dell’istanza.
...
Gli **snapshot di EBS** sono backup incrementali dei volumi EBS. Permettono di salvare una copia dei dati in un determinato momento e di ripristinarli in caso di guasto o perdita. Creare snapshot regolari è una best practice di [[3.1 Sicurezza|sicurezza]] e resilienza, poiché consente di proteggere i dati da errori, danni o cancellazioni accidentali.
...
Amazon S3 può essere utilizzato per l’**hosting statico di siti web**, ovvero pagine [[2.1 HTML|HTML]], CSS, [[3.1 JavaScript|JavaScript]] e media che non richiedono un server backend. Basta caricare i file in un bucket, abilitare l’hosting web statico e ottenere un URL pubblico del sito. È una soluzione semplice, economica e scalabile per blog o pagine aziendali.

## Amazon Elastic [[3.2 File System|File System]] (Amazon EFS)

**Amazon EFS (Elastic [[3.2 File System|File System]])** è un servizio di [[3.2 File System|file system]] gestito e scalabile offerto da AWS. Permette di archiviare e condividere dati tra più istanze EC2 in modo simultaneo, come se fosse un [[3.2 File System|file system]] di rete tradizionale. È ideale per scenari in cui più server o applicazioni devono accedere contemporaneamente agli stessi file, ad esempio per analisi dati, contenuti multimediali o applicazioni condivise.
...
*   **File Gateway (S3 File Gateway)**: permette di archiviare e accedere ai file direttamente in Amazon S3, mantenendo compatibilità con i protocolli di [[3.2 File System|file system]] comuni come NFS e SMB. È ideale per condividere file o centralizzare dati accessibili da più servizi AWS.
...
*   **Affidabilità e [[3.1 Sicurezza|sicurezza]] AWS**: tutti i dati vengono protetti con crittografia e snapshot automatici, assicurando alta durabilità e conformità.
...
Il grande valore di AWS Storage Gateway è che consente di ottenere i benefici del cloud senza abbandonare l’infrastruttura locale. Le aziende possono continuare a lavorare come sempre, ma con l’aggiunta di un livello di [[3.1 Sicurezza|sicurezza]], flessibilità e scalabilità tipico di AWS. In sostanza, è una porta d’ingresso verso il cloud, perfetta per chi vuole iniziare a migrare in modo graduale e senza rischi.
