...
---
## [[1.1 Introduzione ad Amazon Web Services|Introduzione ad Amazon Web Services]]
...
## [[1.2 Infrastruttura Globale e Affidabilità|Infrastruttura Globale e Affidabilità]]
...
## [[2.1 Calcolo nel Cloud|Calcolo nel Cloud]]
...
-   **Prezzi EC2**:
    ...
    -   **Host dedicati**: Intero server fisico per uso esclusivo (licenze, [[3.1 Sicurezza|sicurezza]]).
...
-   **AWS Lambda**: Servizio "serverless" per eseguire codice in risposta a [[2.4 Eventi|eventi]], senza gestire server. Ideale per elaborazioni brevi.
-   **Container (ECS/EKS)**:
...
## [[2.3 Archiviazione e Database|Archiviazione e Database]]

### Archiviazione dell'istanza e Amazon EBS
-   **Archiviazione a livello di blocco**: Memorizza i dati in blocchi (efficiente per DB/[[3.2 File System|file system]]).
...
-   **Hosting statico di siti web**: S3 può ospitare siti [[2.1 HTML|HTML]], CSS, JS senza server backend.

### Amazon Elastic [[3.2 File System|File System]] (Amazon EFS)
-   **Amazon EFS (Elastic [[3.2 File System|File System]])**: [[3.2 File System|File system]] gestito, scalabile, condiviso tra più istanze EC2 (come un [[3.2 File System|file system]] di rete). Autoscalante.
...
-   **Amazon FSx**: [[3.2 File System|File system]] completamente gestiti per specifici carichi di lavoro (es. Windows File Server, Lustre).
...
### Introduzione ai [[2.4 Database|database]]
-   **Modello di responsabilità condivisa AWS**:
    ...
    -   **Non gestiti**: Cliente gestisce tutto (installazione, configurazione, patch, [[3.1 Sicurezza|sicurezza]], backup, HA, ottimizzazione).

### Servizi di [[2.4 Database|database]] relazionali
-   **[[2.4 Database|Database]] relazionale**: Archivia e collega dati tra tabelle diverse (transazionali, strutturati).
-   **Rehosting (lift and shift)**: Migrare [[2.4 Database|database]] esistenti su istanze EC2 senza modifiche strutturali.
-   **Amazon RDS (Relational [[2.4 Database|Database]] Service)**: Servizio gestito per [[2.4 Database|database]] relazionali ([[4.3 MySQL|MySQL]], PostgreSQL, MariaDB, Oracle, SQL Server). AWS gestisce patch, backup, replica, failover.
-   **Amazon Aurora**: Motore relazionale completamente gestito, compatibile con [[4.3 MySQL|MySQL]]/PostgreSQL, 5x più veloce, [[3.3 Architettura|architettura]] distribuita, fino a 15 repliche, backup continui.

### Servizi di [[2.4 Database|database]] NoSQL
-   **[[2.4 Database|Database]] NoSQL**: Organizza i dati in modo non relazionale (es. chiave-valore, documenti). Flessibile per dati non strutturati/semi-strutturati.
-   **Amazon DynamoDB**: Servizio [[2.4 Database|database]] NoSQL completamente gestito (documenti, chiave-valore). Alte prestazioni, scalabilità automatica, tempi di risposta <10ms, disponibilità 99.999%, crittografia completa.
...
### Servizi di [[2.4 Database|database]] aggiuntivi
-   **Amazon DocumentDB**: [[2.4 Database|Database]] completamente gestito compatibile con MongoDB (documenti JSON).
-   **AWS Backup**: Centralizza e automatizza la gestione dei backup AWS e on-premises.
-   **Amazon Neptune**: [[2.4 Database|Database]] a grafo completamente gestito per dati fortemente connessi.

## [[2.5 IA-Machine Learning e Analisi dei Dati|IA-Machine Learning e Analisi dei Dati]]

### Introduzione all'intelligenza artificiale e al [[0.00 Machine Learning|machine learning]]
-   **Intelligenza artificiale (AI)**: Sistemi informatici che svolgono compiti simili a quelli umani.
-   **[[0.00 Machine Learning|Machine Learning]] (ML)**: Sottocampo dell'AI per addestrare macchine a trovare pattern e fare previsioni (modello di ML).
-   **Stack AI/ML AWS**:
    ...
    -   **Servizi di [[0.00 Machine Learning|machine learning]] (ML Services)**: Approccio personalizzato con **Amazon SageMaker AI** (crea, addestra, distribuisce modelli personalizzati).
    -   **Framework e infrastruttura di [[0.00 Machine Learning|machine learning]]**: Controllo massimo con framework (TensorFlow, PyTorch) su infrastruttura AWS (EC2, EMR, ECS).

### Introduzione all'IA generativa su AWS
-   **[[0.00 Deep Learning|Deep Learning]]**: Sottocampo del ML che usa [[2.2 Reti|reti]] neurali artificiali per problemi complessi (immagini, linguaggio).
...
-   **Analisi tradizionale dei dati**: Esamina dati passati per spiegare [[2.4 Eventi|eventi]] e trovare trend.
...
-   **AWS Direct Connect**: Connessione privata dedicata tra data center e VPC (maggiore velocità, minore latenza, più [[3.1 Sicurezza|sicurezza]]).
...
-   **AWS Secrets Manager**: Gestisce, ruota e recupera credenziali del [[2.4 Database|database]], chiavi API e altri segreti.
-   **AWS Systems Manager**: Visualizzazione centralizzata dei nodi, automazione modifiche, gestione patch.

### Protezione di [[2.2 Reti|reti]] e applicazioni
...
-   **Protezioni AWS**: [[3.3 Architettura|Architettura]] globale resistente, gruppi di [[3.1 Sicurezza|sicurezza]], ELB distribuisce traffico.
...
-   **Crittografia**: Essenziale per riservatezza dati.
    -   **Dati inattivi**: Dati fermi ([[2.4 Database|database]], S3, EBS).
    -   **Dati in transito**: Dati tra sistemi (SSL/TLS).
-   **Servizi con crittografia integrata**: Amazon S3, EBS, DynamoDB.
-   **Servizio AWS di [[4.2 Gestione delle Chiavi|gestione delle chiavi]] (KMS)**: Crea, gestisce, controlla chiavi crittografiche.
...
### Rilevamento e risposta agli incidenti di [[3.1 Sicurezza|sicurezza]]
-   **Amazon Inspector**: Valutazioni di [[3.1 Sicurezza|sicurezza]] automatiche (EC2, container, Lambda), identifica vulnerabilità.
-   **Amazon GuardDuty**: Rilevamento intelligente delle minacce monitorando flussi metadati/attività di rete.
-   **Amazon Detective**: Indaga su incidenti di [[3.1 Sicurezza|sicurezza]], mostra relazioni tra risorse/utenti.
-   **Centrale di [[3.1 Sicurezza|sicurezza]] AWS (AWS Security Hub)**: Aggrega risultati di [[3.1 Sicurezza|sicurezza]] da vari servizi, visione completa dello stato di [[3.1 Sicurezza|sicurezza]].

## [[3.2 Monitoraggio, Conformità e Governance nel Cloud AWS|Monitoraggio, Conformità e Governance nel Cloud AWS]]

### Importanza del monitoraggio
-   **Monitoraggio**: Osservare e analizzare attività del sistema, traffico, [[2.4 Eventi|eventi]] di [[3.1 Sicurezza|sicurezza]].
-   **Amazon CloudWatch**: Servizio di monitoraggio in tempo reale su risorse AWS e applicazioni.
    ...
    -   **Dashboard CloudWatch**: [[3.4 Viste|Viste]] personalizzate.
...
-   **AWS CloudTrail**: Traccia attività utenti e utilizzo API (chi, cosa, quando, su quali risorse).
    -   **[[2.4 Eventi|Eventi]] CloudTrail**: Registrano azioni (API, console). Cronologia 90 giorni gratuita.
...
-   **AWS Artifact**: Accesso on-demand a report AWS di [[3.1 Sicurezza|sicurezza]] e conformità (accordi, report ISO, SOC, PCI, HIPAA).
...
-   **AWS Organizations**: Gestisce centralmente più account AWS.
    -   **Vantaggi**: Scalabilità rapida, gestione policy centralizzata, ottimizzazione costi (fatturazione consolidata), controllo [[3.1 Sicurezza|sicurezza]].
...
### Governance
-   **AWS Control Tower**: Applica e gestisce regole di governance per [[3.1 Sicurezza|sicurezza]], operazioni e conformità su larga scala.
...
-   **AWS Health Dashboard**: Visualizza informazioni sull'integrità specifiche dell'account e aggiornamenti [[2.4 Eventi|eventi]].
-   **AWS Trusted Advisor**: Valuta ambiente AWS ([[3.1 Sicurezza|sicurezza]], prestazioni, costi, resilienza) rispetto a best practice, fornisce raccomandazioni.
-   **Sistema di analisi degli accessi IAM**: Analizza permessi, individua eccessi di privilegi, suggerisce ottimizzazioni.

## [[2.6 Migrazione al Cloud AWS|Migrazione al Cloud AWS]]
...
-   **Sei prospettive**: Azienda, Persone, Governance, Piattaforma, [[3.1 Sicurezza|Sicurezza]], Operazioni.
...
### Migrazioni dei [[2.4 Database|database]]
-   **AWS [[2.4 Database|Database]] Migration Service (DMS)**: Trasferisce [[2.4 Database|database]]/data warehouse in modo rapido, sicuro, con tempi di inattività minimi. Supporta migrazioni omogenee ed eterogenee.
-   **AWS Schema Conversion Tool (SCT)**: Automatizza conversione schemi [[2.4 Database|database]] per migrazioni eterogenee.
...
## [[4.1 Prezzi e Supporto|Prezzi e Supporto]]
...
## [[5.1 Soluzioni Well-Architected|Soluzioni Well-Architected]]
...
-   **Sei pilastri**:
    ...
    -   **[[3.1 Sicurezza|Sicurezza]]**: Protezione dati/sistemi (privilegio minimo, crittografia, integrità).
...
### [[2.2 Casi d'uso|Casi d'uso]] specializzati
