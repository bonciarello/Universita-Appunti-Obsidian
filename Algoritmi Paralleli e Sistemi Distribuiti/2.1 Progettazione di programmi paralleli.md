...
In [[3.1 OpenMP|OpenMP]] è piuttosto fattibile grazie alla **ricorsione**.
...
- **nell'[[3.3 Architettura|architettura]] della memoria distribuita:** se o quando il valore di *X* viene comunicato tra le attività.
- **nell'[[3.3 Architettura|architettura]] della memoria condivisa:** quale attività memorizza il valore di *X* per ultimo.
...
Funziona meglio se applicato a un'[[3.3 Architettura|architettura]] di memoria condivisa.
...
### Esempio: query su [[2.4 Database|database]]
...
> **NOTA:** ricorda che ci riferiamo alla mappatura tra tasks e processi e non ai processori. Questo perché, d'altra parte, le API utilizzate tipiche (ad esempio [[3.1 OpenMP|OpenMP]], MPI) non consentono un facile collegamento delle tasks ai processori fisici. Piuttosto, cercano di aggregare i tasks ai processi, **dando al sistema il compito di mappare i processi ai processori (per efficienza)**.
...
Adatto per algoritmi *[[2.3 Divide et impera|divide et impera]]*.
...
La concorrenza aumenta con la profondità dell'albero. Inizialmente è necessario un solo processo per la prima partizione. A volte è meglio formulare l'algoritmo in termini di *[[2.3 Divide et impera|divide et impera]]* anche se non è intrinsecamente di questo tipo.
