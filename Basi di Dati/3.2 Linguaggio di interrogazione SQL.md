...
tags: [basi-di-dati]
---
**SQL (Structured Query Language)** è un linguaggio standardizzato per [[2.4 Database|database]] basati sul modello relazionale (RDBMS), progettato per le seguenti operazioni:

- creare e modificare schemi di [[2.4 Database|database]] *(DDL = Data Definition Language)*;
...
A dispetto del nome, non si tratta perciò di un semplice linguaggio di interrogazione: alcuni suoi sottoinsiemi, infatti, permettono di creare, gestire e amministrare [[2.4 Database|database]].
...
Sono funzioni non rappresentabili in [[3.1 Algebra relazionale|Algebra Relazionale]], quindi rendono SQL più espressivo dell’AR. Sono: *COUNT*, *SUM*, *MIN*, *MAX*, *AVG*.
