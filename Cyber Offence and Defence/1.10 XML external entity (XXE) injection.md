...
---
La XML external entity injection (nota anche come XXE) è una vulnerabilità di [[3.1 Sicurezza|sicurezza]] web che consente a un aggressore di interferire con l'elaborazione di dati XML da parte di un'applicazione. Spesso consente a un aggressore di visualizzare file sul [[3.2 File System|file system]] del server applicativo e di interagire con qualsiasi sistema back-end o esterno a cui l'applicazione stessa può accedere.
...
Le entità esterne XML sono un tipo di entità XML personalizzata i cui valori definiti vengono caricati dall'esterno del DTD in cui sono dichiarati. Le entità esterne sono particolarmente interessanti dal punto di vista della [[3.1 Sicurezza|sicurezza]] perché consentono di definire un'entità in base al contenuto di un percorso di file o di un URL.
...
Per eseguire un attacco di XXE injection che recupera un file arbitrario dal [[3.2 File System|file system]] del server, è necessario modificare l'XML inviato in due modi:
