...
I controlli degli accessi non funzionanti sono comuni e spesso presentano una vulnerabilità critica alla [[3.1 Sicurezza|sicurezza]]. La progettazione e la gestione dei controlli degli accessi sono un problema complesso e dinamico che applica vincoli aziendali, organizzativi e legali a un'implementazione tecnica. Le decisioni sulla progettazione del controllo degli accessi devono essere prese da esseri umani; quindi, il potenziale di errori è elevato.
...
Con i controlli di accesso verticali, diversi tipi di utenti hanno accesso a diverse funzioni dell'applicazione. Ad esempio, un amministratore potrebbe essere in grado di modificare o eliminare l'account di qualsiasi utente, mentre un utente normale non ha accesso a queste azioni. I controlli di accesso verticali possono essere implementazioni più dettagliate di modelli di [[3.1 Sicurezza|sicurezza]] progettati per applicare policy aziendali come la separazione dei compiti e il privilegio minimo.
...
Questo potrebbe essere accessibile a qualsiasi utente, non solo agli utenti amministrativi che hanno un collegamento alla funzionalità nella loro [[2.1 Interfaccia utente|interfaccia utente]]. In alcuni casi, l'URL amministrativo potrebbe essere divulgato in altre posizioni, come il file `robots.txt`:
...
In alcuni casi, la funzionalità sensibile viene nascosta assegnandole un URL meno prevedibile. Questo è un esempio della cosiddetta "[[3.1 Sicurezza|sicurezza]] per oscurità". Tuttavia, nascondere la funzionalità sensibile non fornisce un controllo di accesso efficace perché gli utenti potrebbero scoprire l'URL offuscato in vari modi.

Immagina un'applicazione che ospita funzioni amministrative al seguente URL:
`https://insecure-website.com/administrator-panel-yb556`

Questo potrebbe non essere direttamente indovinabile da un aggressore. Tuttavia, l'applicazione potrebbe comunque far trapelare l'URL agli utenti. L'URL potrebbe essere divulgato in [[3.1 JavaScript|JavaScript]] che costruisce l'[[2.1 Interfaccia utente|interfaccia utente]] in base al ruolo dell'utente:
...
Questo script aggiunge un collegamento all'[[2.1 Interfaccia utente|interfaccia utente]] dell'utente se è un utente amministratore. Tuttavia, lo script contenente l'URL è visibile a tutti gli utenti, indipendentemente dal loro ruolo (APPRENTICE: Unprotected admin functionality with unpredictable URL).
...
**Vulnerabilità IDOR con riferimento diretto agli oggetti del [[2.4 Database|database]]** Considera un sito web che utilizza il seguente URL per accedere alla pagina dell'account del cliente, recuperando le informazioni dal [[2.4 Database|database]] back-end:
`https://insecure-website.com/customer_account?customer_number=132355`

Qui, il numero del cliente viene utilizzato direttamente come indice di record nelle query eseguite sul [[2.4 Database|database]] back-end. Se non sono presenti altri controlli, un aggressore può semplicemente modificare il valore `customer_number`, aggirando i controlli di accesso per visualizzare i record di altri clienti. Questo è un esempio di vulnerabilità IDOR che porta all'escalation dei privilegi orizzontali.
