...
- **Stored XSS** in cui lo script dannoso proviene dal [[2.4 Database|database]] del sito web;
...
- **determina il contesto di riflessione:** per ogni posizione all'interno della risposta in cui viene riflesso il valore casuale, determina il suo contesto. Questo potrebbe essere nel testo tra tag [[2.1 HTML|HTML]], all'interno di un attributo tag che potrebbe essere tra virgolette, all'interno di una stringa [[3.1 JavaScript|JavaScript]], ecc;
...
## DOM-based XSS
Le vulnerabilità DOM-based XSS si verificano solitamente quando [[3.1 JavaScript|JavaScript]] prende dati da una fonte controllabile dall'aggressore, come l'URL, e li passa a un sink che supporta l'esecuzione di codice dinamico, come `eval()` o `innerHTML`. Ciò consente agli aggressori di eseguire [[3.1 JavaScript|JavaScript]] dannoso, che in genere consente loro di dirottare gli account di altri utenti.

Per eseguire un attacco DOM-based XSS, è necessario inserire i dati in una fonte in modo che vengano propagati a un sink e provochino l'esecuzione di [[3.1 JavaScript|JavaScript]] arbitrario.
...
- **test dei “sink” [[2.1 HTML|HTML]]:** per verificare la presenza di DOM XSS nei “sink” [[2.1 HTML|HTML]], si inserisce una stringa casuale nella fonte (ad esempio, `location.search`) e si utilizza lo strumento di ispezione [[2.1 HTML|HTML]] per individuare dove compare la stringa. Il comando “View source” non è utile per il DOM XSS, poiché non mostra le modifiche apportate da [[3.1 JavaScript|JavaScript]]; invece, negli strumenti per sviluppatori di Chrome è possibile cercare la stringa direttamente nel DOM. Per ogni posizione in cui la stringa appare, si deve identificare il contesto e affinare l’input per verificare come viene elaborato;
- **test dei “sink” di esecuzione [[3.1 JavaScript|JavaScript]]:** testare i “sink” di esecuzione [[3.1 JavaScript|JavaScript]] per il DOM XSS è più complesso, poiché l’input potrebbe non apparire nel DOM. È necessario esaminare il codice [[3.1 JavaScript|JavaScript]] con il debugger, aggiungere un breakpoint dove viene letto il “source” e monitorare l’uso della variabile fino al “sink”. A questo punto, si può affinare l’input per verificare se è possibile eseguire un attacco XSS.
...
Si noti, tuttavia, che in alcune situazioni il contenuto scritto in `document.write` include un contesto circostante di cui è necessario tenere conto nel proprio exploit. Ad esempio, potrebbe essere necessario chiudere alcuni elementi esistenti prima di utilizzare il payload [[3.1 JavaScript|JavaScript]].

Il sink `innerHTML` non accetta elementi script su nessun browser moderno, né gli [[2.4 Eventi|eventi]] svg `onload` vengono attivati. Ciò significa che dovrai usare elementi alternativi come `img` o `iframe`. I gestori di [[2.4 Eventi|eventi]] come `onload` e `onerror` possono essere usati insieme a questi elementi. Ad esempio:
...
**DOM XSS in [[3.4 jQuery|jQuery]]:** se si utilizza una libreria [[3.1 JavaScript|JavaScript]] come [[3.4 jQuery|jQuery]], bisogna fare attenzione ai sink che possono modificare gli elementi DOM sulla pagina. Ad esempio, la funzione `attr()` di [[3.4 jQuery|jQuery]] può modificare gli attributi degli elementi DOM. Se i dati vengono letti da una fonte controllata dall'utente come l'URL, quindi passati alla funzione `attr()`, potrebbe essere possibile manipolare il valore inviato per causare XSS. Ad esempio, qui abbiamo un po' di [[3.1 JavaScript|JavaScript]] che modifica l'attributo `href` di un elemento di ancoraggio utilizzando i dati dall'URL:
```javascript
$(function() {
$('#backLink').attr("href",(new URLSearchParams(window.location.search)).get('returnUrl'));
});
```
Puoi sfruttarlo modificando l'URL in modo che la sorgente `location.search` contenga un URL [[3.1 JavaScript|JavaScript]] dannoso. Dopo che il [[3.1 JavaScript|JavaScript]] della pagina applica questo URL dannoso all'href del back link, cliccando sul back link verrà eseguito:
`?returnUrl=javascript:alert(document.domain)`

Un altro potenziale sink da tenere d'occhio è la funzione selettore `$()` di [[3.4 jQuery|jQuery]], che può essere utilizzata per iniettare oggetti dannosi nel DOM. [[3.4 jQuery|jQuery]] era molto popolare e una classica vulnerabilità DOM XSS era causata da siti web che utilizzavano questo selettore insieme alla sorgente `location.hash` per le [[5.3 Animazioni|animazioni]] o lo scorrimento automatico verso un elemento particolare sulla pagina. Questo comportamento veniva spesso implementato utilizzando un gestore di [[2.4 Eventi|eventi]] `hashchange` vulnerabile, simile al seguente:
```javascript
...
```
Poiché l'hash è controllabile dall'utente, un aggressore potrebbe usarlo per iniettare un vettore XSS nel sink del selettore `$()`. Versioni più recenti di [[3.4 jQuery|jQuery]] hanno corretto questa particolare vulnerabilità impedendoti di iniettare [[2.1 HTML|HTML]] in un selettore quando l'input inizia con un carattere hash (#). Tuttavia, potresti comunque trovare codice vulnerabile in natura.
...
**DOM XSS in AngularJS:** se si utilizza un framework come AngularJS, potrebbe essere possibile eseguire [[3.1 JavaScript|JavaScript]] senza parentesi angolari o [[2.4 Eventi|eventi]]. Quando un sito utilizza l'attributo `ng-app` su un elemento [[2.1 HTML|HTML]], verrà elaborato da AngularJS. In questo caso, AngularJS eseguirà [[3.1 JavaScript|JavaScript]] all'interno di doppie parentesi graffe che possono verificarsi direttamente in [[2.1 HTML|HTML]] o all'interno di attributi.
...
Tuttavia, le fonti non sono limitate ai dati direttamente esposti dai browser, possono anche provenire dal sito web. Ad esempio, i siti web spesso riflettono i parametri URL nella risposta [[2.1 HTML|HTML]] dal server. Ciò è comunemente associato a XSS normale, ma può anche portare a vulnerabilità DOM XSS reflected.

In una vulnerabilità DOM XSS reflected, il server elabora i dati dalla richiesta e li riecheggia nella risposta. I dati riflessi potrebbero essere inseriti in una stringa letterale [[3.1 JavaScript|JavaScript]] o in un elemento dati all'interno del DOM, come un campo di un modulo. Uno script sulla pagina elabora quindi i dati riflessi in modo non sicuro, scrivendoli infine in un sink pericoloso.
...
Il modo tradizionale per dimostrare di aver trovato una vulnerabilità di cross-site scripting è creare un popup usando la funzione `alert()`. Questo non perché XSS abbia qualcosa a che fare con i popup; è semplicemente un modo per dimostrare che puoi eseguire [[3.1 JavaScript|JavaScript]] arbitrario su un dato dominio. Potresti notare che alcune persone usano `alert(document.domain)`. Questo è un modo per rendere esplicito su quale dominio è in esecuzione [[3.1 JavaScript|JavaScript]].
...
- molte applicazioni nascondono i loro cookie da [[3.1 JavaScript|JavaScript]] utilizzando il flag `HttpOnly`;
...
Supponiamo anche che l'applicazione non filtri o esegua l'escape dei caratteri `>` o `"`. Un aggressore può usare la seguente sintassi per uscire dal valore dell'attributo tra virgolette e dal tag di chiusura e tornare a un contesto [[2.1 HTML|HTML]]:
`">`

In questa situazione, un aggressore tenterebbe naturalmente di eseguire XSS. Ma supponiamo che un normale attacco XSS non sia possibile, a causa di filtri di input, policy di [[3.1 Sicurezza|sicurezza]] dei contenuti o altri ostacoli. Qui, potrebbe essere ancora possibile eseguire un attacco di dangling markup injection utilizzando un payload come il seguente:
...
CSP è un meccanismo di [[3.1 Sicurezza|sicurezza]] del browser che mira a mitigare XSS e altri attacchi. Funziona limitando le risorse (come script e immagini) che una pagina può caricare e limitando se una pagina può essere inquadrata da altre pagine.
...
Utilizzare la policy di [[3.1 Sicurezza|sicurezza]] dei contenuti per impedire il [[2.4 Clickjacking|clickjacking]] è più flessibile rispetto all'utilizzo dell'intestazione `X-Frame-Options` perché è possibile specificare più domini e utilizzare caratteri jolly. Ad esempio:
...
Di solito, non è possibile sovrascrivere una direttiva `script-src` esistente. Tuttavia, Chrome ha recentemente introdotto la direttiva `script-src-elem`, che consente di controllare gli elementi dello script, ma non gli [[2.4 Eventi|eventi]]. Fondamentalmente, questa nuova direttiva consente di sovrascrivere le direttive `script-src` esistenti.
...
- **codifica i dati in output:** nel punto in cui i dati controllabili dall'utente vengono emessi nelle risposte HTTP, codifica l'output per evitare che venga interpretato come contenuto attivo. A seconda del contesto di output, potrebbe essere necessario applicare combinazioni di codifica [[2.1 HTML|HTML]], URL, [[3.1 JavaScript|JavaScript]] e CSS;
- **utilizza intestazioni di risposta appropriate:** per prevenire XSS nelle risposte HTTP che non sono destinate a contenere [[2.1 HTML|HTML]] o [[3.1 JavaScript|JavaScript]], puoi utilizzare le intestazioni `Content-Type` e `X-Content-Type-Options` per assicurarti che i browser interpretino le risposte nel modo desiderato;
- **politica di [[3.1 Sicurezza|sicurezza]] dei contenuti:** come ultima linea di difesa, è possibile utilizzare Content Security Policy (CSP) per ridurre la gravità delle vulnerabilità XSS ancora presenti.