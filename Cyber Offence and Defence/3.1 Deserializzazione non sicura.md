...
- scrivere dati complessi nella memoria interprocesso, in un file o in un [[2.4 Database|database]]
...
### Magic methods
I magic methods sono metodi speciali che vengono invocati automaticamente al verificarsi di determinati [[2.4 Eventi|eventi]], senza bisogno di richiamarli esplicitamente. Questi metodi, comuni nella programmazione orientata agli oggetti, sono spesso identificati da doppie underscore nel nome (ad esempio, `__construct()` in PHP o `__init__()` in Python).

I magic method possono essere personalizzati dagli sviluppatori per eseguire codice specifico in risposta a [[2.4 Eventi|eventi]], come l’istanziazione di un oggetto (`__construct()` in PHP) o il caricamento di dati durante la deserializzazione (`__wakeup()` in PHP e `readObject()` in Java). In Java, i metodi come `readObject()` permettono alle classi di controllare come i campi vengono deserializzati.
