...
Uno degli altri problemi chiave con OAuth è la generale mancanza di funzionalità di [[3.1 Sicurezza|sicurezza]] integrate. La [[3.1 Sicurezza|sicurezza]] si basa quasi interamente sugli sviluppatori che utilizzano la giusta combinazione di opzioni di configurazione e implementano le proprie misure di [[3.1 Sicurezza|sicurezza]] aggiuntive in aggiunta, come una convalida di input robusta.
...
Tuttavia, questa semplicità ha un costo in termini di [[3.1 Sicurezza|sicurezza]]. Poiché tutte le comunicazioni avvengono tramite reindirizzamenti del browser e manca un back-channel sicuro, il token di accesso è più vulnerabile a eventuali attacchi. Questo tipo di concessione è più indicato per applicazioni a pagina singola e applicazioni desktop native, dove la gestione di un `client_secret` sul back-end è complessa o impossibile.
...
Per prevenire vulnerabilità OAuth, sia i provider che le app client devono implementare controlli robusti, specialmente sul parametro `redirect_uri`. L’OAuth offre poca protezione integrata, quindi la [[3.1 Sicurezza|sicurezza]] dipende dagli sviluppatori.
