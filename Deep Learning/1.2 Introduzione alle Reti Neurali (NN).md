...
Nella definizione più generale, le **[[0.00 Reti|Reti]] Neurali (NN)**, spesso chiamate semplicemente [[0.00 Reti|reti]] neurali artificiali, sono modelli matematici e computazionali ispirati al funzionamento dei neuroni nel cervello umano.
...
Le [[0.00 Reti|reti]] neurali apprendono dai dati mediante l'addestramento, regolando i pesi delle connessioni sinaptiche per minimizzare l'errore tra l'output previsto e quello desiderato.
...
*   **ReLU (Rectified Linear Unit):** $f(x) = \max(0, x)$. Restituisce l'input se positivo, 0 altrimenti. Molto usata nel [[0.00 Deep Learning|Deep Learning]] per la sua semplicità e gestione del gradiente.
*   **Tanh (Tangente iperbolica):** Mappa l'input tra -1 e 1.

### Struttura della Rete (Feedforward)
Nelle [[0.00 Reti|reti]] **Feedforward**, l'informazione fluisce in una sola direzione (input $\to$ hidden $\to$ output) senza cicli.
...
**Per la [[2.2 Regressione|Regressione]]:**
...
*   **Inizializzazione di He:** Ottimizzata per [[0.00 Reti|reti]] con attivazione ReLU. Imposta la varianza per mantenere il flusso del segnale stabile.
