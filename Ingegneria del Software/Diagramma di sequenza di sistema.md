...
Un **diagramma di sequenza di sistema** è una figura che mostra, *per un particolare scenario di un caso d'uso*, gli [[Eventi|eventi]] generati da attori esterni, il loro ordine e gli [[Eventi|eventi]] inter-sistema (ovvero, tra sistemi). Tutti i sistemi sono considerati a scatola nera; il diagramma enfatizza gli [[Eventi|eventi]] che superano i confini sei sistemi, dagli attori ai sistemi.
...
Una domanda interessante e utile nella progettazione del software è la seguente: *Quali [[Eventi|eventi]] solleciteranno il nostro sistema?* É utile e interessante perché il software deve essere progettato per gestire questi [[Eventi|eventi]] (dal mouse, dalla tastiera, da un altro sistema, ...) e generare delle risposte. Fondamentalmente, un sistema software reagisce a tre cose:

1. [[Eventi|eventi]] esterni da parte di attori (umani o sistemi informatici);
2. [[Eventi|eventi]] di timer;
3. guasti o eccezioni (spesso provenienti da sorgenti esterne).

Pertanto è utile sapere, *con precisione*, quali sono gli [[Eventi|eventi]] esterni di input, ovvero gli **[[Eventi|eventi]] di sistema**. Essi rappresentano una parte importante dell'analisi del comportamento di un sistema.
...
Un SSD mostra gli [[Eventi|eventi]] di sistema *per un solo scenario di un caso d'uso*, e pertanto è generato per ispezione da un caso d'uso (come in figura).
