...
## I thread e [[3.1 JavaFX|JavaFX]]

Lo Scene Graph di [[3.1 JavaFX|JavaFX]] non è thread-safe e può essere letto e modificato solo dal thread dell'interfaccia grafica, detto **[[3.1 JavaFX|JavaFX]] Application thread**. Quindi, nelle applicazioni [[3.1 JavaFX|JavaFX]] non è possibile usare dei thread che modificano l'interfaccia grafica come nel caso delle Swing.

Nel caso in cui si voglia implementare una porzione di codice che funziona in parallelo rispetto all'interfaccia grafica, si possono utilizzare le API definite nel package *[[3.1 JavaFX|javafx]].concurrent*. Queste API si occupano di interagire con l'interfaccia grafica di [[3.1 JavaFX|JavaFX]], assicurandosi che l'interazione avvenga nel thread corretto.

## Il package [[3.1 JavaFX|javafx]].concurrent

Il package [[3.1 JavaFX|javafx]].concurrent consiste dell'interfaccia Worker e due classi, Task e Service, che implementano l'interfaccia Worker.

La classe Task permette agli sviluppatori di sviluppare task asincroni in applicazioni [[3.1 JavaFX|JavaFX]]. La classe Service esegue i task. Inoltre, le classi Task e Service implementano l'interfaccia EventTarget e questo permette di ricevere notifiche quando lo stato di un Worker cambia.

## L'interfaccia Worker

Definisce un oggetto che esegue codice su uno o più thread in background. Lo stato dell'oggetto Worker è osservabile e usabile dal [[3.1 JavaFX|JavaFX]] Application thread. Gli stati di un Worker sono i seguenti:
...
La classe Service è progettata per eseguire un oggetto di tipo Task su uno o più thread in background. Lo scopo di questa classe è di aiutare lo sviluppatore ad implementare la corretta interazione tra i thread in background e il [[3.1 JavaFX|JavaFX]] Application thread.
