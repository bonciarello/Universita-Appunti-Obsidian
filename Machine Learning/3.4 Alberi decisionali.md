...
È un algoritmo utilizzato per la classificazione, viene applicato ricorsivamente. Ad ogni step viene scelto il **best-attribute**, per effettuare lo split. Il *best-attribute* sarà quello con la maggiore differenza di [[3.3 Entropia|entropia]] prima e dopo lo split, perchè il nostro obiettivo è quello di formare alberi decisionali bilanciati e semplici, generando sotto-alberi (sottoinsiemi) puri.

I sottoinsiemi puri sono quelli in cui compare per tutti gli esempi la stessa *class label*.

La purezza (concetto legato all'[[3.3 Entropia|entropia]]) si misura con l'**Information Gain** o il **Gini Index**.
...
## Information Gain
L'**Information Gain *IG(S,A)*** è la prevista riduzione di [[3.3 Entropia|entropia]] causata dal partizionamento degli esempi del training set *S* secondo l'attributo *A*.

> $IG(S,A) = E(S) - \sum_{v \in values(A)} \frac{\left | S_v \right |}{\left | S \right |} E(S_v)$

dove:
- $E(S)$ è l'[[3.3 Entropia|entropia]] di *S* (prima della divisione);
- $E(S_v)$ è l'[[3.3 Entropia|entropia]] del sottoinsieme $S_v$ di *S* dove $A=v$ (dopo la divisione)
...
Riassumendo:
- *IG(S,A)* è la prevista riduzione di [[3.3 Entropia|entropia]] causata dal partizionamento degli esempi di *S* secondo l'attributo *A*;
...
Può essere utilizzata come alternativa all’[[3.3 Entropia|entropia]].
