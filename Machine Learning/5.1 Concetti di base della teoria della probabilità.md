...
$X=1$ e $Y=1$ sono [[2.4 Eventi|eventi]] indipendenti, in quanto la probabilità di $X=1$ non è influenzata dalla condizione $Y=1$. Allora:
- $p(X=1) = 0.5$;
- $p(X=1|Y=1) = 0.5$.
...
In effetti, "essere alcolizzati" e "malattie del fegato" NON sono [[2.4 Eventi|eventi]] indipendenti.
...
Avremo $p(A=1 + B=1) = p(A=1) + p(B=1) - p(A=1,B=1)$. Poiché $A=1$ e $B=1$ sono [[2.4 Eventi|eventi]] indipendenti, allora $p(A=1,B=1) = p(A=1)*p(B=1) = \frac{1}{5}$. Quindi $p(A=1 + B=1) = \frac{7}{10}$.
