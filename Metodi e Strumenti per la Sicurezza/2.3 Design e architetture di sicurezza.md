...
L'approccio corretto consiste nell'integrare la [[3.1 Sicurezza|sicurezza]] fin dall'inizio (**Security by Design**), non come un'aggiunta finale.
Le quattro fasi principali sono:
1.  **Requisiti:** Definire cosa proteggere.
2.  **Progetto:** Scegliere l'[[3.3 Architettura|architettura]].
...
*   **Hardware:** Difficile da cambiare ma offre vantaggi di [[3.1 Sicurezza|sicurezza]] non facilmente bypassabili.
...
A differenza dei [[2.2 Casi d'uso|casi d'uso]], descrivono ciò che il sistema **non deve permettere**.
...
### Miti della [[3.1 Sicurezza|Sicurezza]]
...
*   **"Il cloud è sicuro di per sé":** Falso. La [[3.1 Sicurezza|sicurezza]] è una responsabilità condivisa (SLA).
*   **"L'API interna è nascosta":** Falso. Se l'app web può accedervi, un attaccante che controlla l'app può fare lo stesso.

## 4. [[3.3 Architettura|Architettura]] di [[3.1 Sicurezza|Sicurezza]] di Google
...
*   **Titan (Chip di [[3.1 Sicurezza|sicurezza]]):** Ogni macchina Google ha un chip Titan che firma l'hash del BIOS e del SO all'avvio per garantire che l'infrastruttura sia fidata.
