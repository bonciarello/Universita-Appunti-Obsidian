...
## 1. Proprietà
Il tipo di funzione hash necessaria per le applicazioni di [[3.1 Sicurezza|sicurezza]] viene definita **funzione hash crittografica** per il quale è computazionalmente impossibile trovare:
...
> **Commentato [CP4]:** La proprietà delle collisioni nelle funzioni hash si riferisce alla possibilità che due diversi input producano lo stesso hash. Una buona funzione hash minimizza le collisioni, rendendole rare, poiché le collisioni possono compromettere la [[3.1 Sicurezza|sicurezza]] e l’integrità dei dati.
...
1.  Il messaggio e il codice hash concatenato viene crittografato utilizzando la [[2.1 Crittografia Simmetrica|crittografia simmetrica]]: poiché solo A e B condividono la chiave segreta, il messaggio deve provenire da A e non è stato alterato;
2.  Solo il digest viene crittografato, utilizzando la [[2.1 Crittografia Simmetrica|crittografia simmetrica]];
...
L’HMAC garantisce che il messaggio non sia stato alterato e che provenga da una fonte autentica ed è ampiamente utilizzato in diversi protocolli e applicazioni che richiedono [[3.1 Sicurezza|sicurezza]] dei dati.
