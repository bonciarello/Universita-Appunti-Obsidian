...
---
Implementando la [[3.1 Sicurezza|sicurezza]] a livello IP, un'organizzazione può garantire una rete sicura per le numerose applicazioni che ignorano la [[3.1 Sicurezza|sicurezza]]. La [[3.1 Sicurezza|sicurezza]] a livello IP (IPSec) comprende tre aree funzionali:
...
Uno dei concetti fondamentali per il funzionamento di IPSec è il concetto di una politica di [[3.1 Sicurezza|sicurezza]] applicata a ciascun pacchetto IP che transita da una sorgente a una destinazione. La politica IPsec è determinata principalmente dall'interazione di due [[2.4 Database|database]], il **Security Association [[2.4 Database|Database]] (SAD)** e il **Security Policy [[2.4 Database|Database]] (SPD)**.

## 1. Security Association (SA)
Il **Security Association (SA)** è una connessione logica unidirezionale tra un mittente e un destinatario che offre servizi di [[3.1 Sicurezza|sicurezza]] al traffico trasportato su di essa. Se è necessaria una relazione peer-to-peer per lo scambio sicuro bidirezionale, sono necessarie due SA.
...
*   **Security Protocol Identifier:** questo campo dell'intestazione IP esterna indica se l'associazione è un'associazione di [[3.1 Sicurezza|sicurezza]] AH o ESP.

In ogni implementazione IPsec, è presente un [[2.4 Database|database]] di SA nominali che definisce i parametri associati a ciascuna SA. Una SA è normalmente definita dai seguenti parametri:
...
> **Commentato [CP15]:** Security Association [[2.4 Database|Database]] (SAD) contiene le informazioni sulle Security Association (SA) attive, che definiscono i parametri di [[3.1 Sicurezza|sicurezza]] per le connessioni IPsec.

## 2. Security Policy [[2.4 Database|Database]] (SPD)
Il mezzo con cui il traffico IP è correlato a SA specifiche (o nessuna SA nel caso di traffico autorizzato a bypassare IPsec) è il **Security Policy [[2.4 Database|Database]] (SPD)** nominale. Un SPD contiene voci, ognuna delle quali definisce un sottoinsieme di traffico IP e punta a una SA per quel traffico. Ogni voce SPD è definita da un insieme di valori di campo IP e protocollo di livello superiore, chiamati **selettori**. Questi selettori vengono utilizzati per filtrare il traffico in uscita al fine di mapparlo in una particolare SA.
...
> **Commentato [CP16]:** Security Policy [[2.4 Database|Database]] (SPD) contiene le politiche di [[3.1 Sicurezza|sicurezza]] che determinano come il traffico di rete deve essere protetto.
...
*   **Modalità tunnel:** La modalità tunnel fornisce protezione all'intero pacchetto IP. Per ottenere ciò, dopo che i campi AH o ESP sono stati aggiunti al pacchetto IP, l'intero pacchetto più i campi di [[3.1 Sicurezza|sicurezza]] vengono trattati con una nuova intestazione IP esterna. L'intero pacchetto originale, interno, viaggia attraverso un tunnel da un punto all'altro di una rete IP; nessun router lungo il percorso è in grado di esaminare l'intestazione IP interna. Poiché il pacchetto originale è incapsulato, il nuovo pacchetto più grande potrebbe avere indirizzi di origine e destinazione completamente diversi. La modalità tunnel viene utilizzata quando una o entrambe le estremità di una SA sono un gateway di [[3.1 Sicurezza|sicurezza]].
