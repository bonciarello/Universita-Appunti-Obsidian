...
*   **Digital storage:** pratica di memorizzare le password in forma digitale, utilizzando appositi strumenti o applicazioni per la gestione delle password:
    *   **Local password managers:** non così bene perché di solito sono sotto attacco. LPM deve ancora fare affidamento sulla memoria umana perché si sblocca il [[2.4 Database|database]] solo se l'utente inserisce la chiave corretta per aprirlo;
    *   **USB password managers:** possono essere utilizzati insieme agli LPM per evitare la possibilità che il [[2.4 Database|database]] interno di LPM possa essere rubato e compromesso. Il [[2.4 Database|database]] viene memorizzato nella penna USB e viene inserito nella porta solo quando è necessario.
...
2.  A questo punto Steve fetcha il proprio [[2.4 Database|database]] e cerca il salt e il verifier associato a quell’account. Il salt viene inviato a Carol la quale computerà la chiave privata sfruttando l’informazione appena inviata e la sua password reale;
