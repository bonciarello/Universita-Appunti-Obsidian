...
tags: [reti]
---
L'**HTTP (Hypertext Transfer Protocol)** nasce per trasferire risorse ipertestuali ([[2.1 HTML|HTML]]). Una pagina [[2.1 HTML|HTML]] non è una pagina a se stanti ed è piena di riferimenti ad altre pagine (oggetti). Ogni oggetto è riferito tramite un URL (con standard RFC 2396, 3986) composto da protocollo, nome dell'host (*FQDN*) ed eventuale nome del percorso.

Il [[1.2 Protocollo HTTP|protocollo HTTP]] funziona su modello **client/server**:
...
Il [[1.2 Protocollo HTTP|protocollo HTTP]] è **stateful** cioè non ci sono normalmente informazioni sulle precedenti connessioni. Il concetto di **sessione** (basata sui *cookies*) è stato aggiunto in seguito.
...
L'*HTTP 3* ha lo scopo di abbattere latenza per richieste multiple e si utilizza per la prima volta l'UDP. Sfrutta un protocollo chiamato QUIC basato su UDP, con un layer di [[3.1 Sicurezza|sicurezza]] TLS, per avere latenze minori.
...
- *HTTP 3:* è multistream cioè la connessione è divisa in stream indipendenti. Sfrutta un protocollo chiamato QUIC basato su UDP, con un layer di [[3.1 Sicurezza|sicurezza]] TLS, per avere latenze minori.  
...
- l'**HTTP response** è composta da una linea di stato (codice di errorre e frase), da un intestazione e da dati (come per esempio file [[2.1 HTML|HTML]]).
...
Il campo dei cookies è presente sia nei messaggi di risposta sia nei messaggi di richiesta. Il browser salva i cookies nei messaggi di risposta e li reinvia la volta successiva che chiede lo stesso oggetto. Il sito web contiene invece un suo [[2.4 Database|database]] dei cookie inviati a tutti i client.

*Facciamo un esempio:* Susanna accede a Internet sempre dallo stesso PC, visita un certo sito di e-commerce e, alla prima richiesta HTTP, il web server associa un ID all’IP di Susanna e lo salva nel [[2.4 Database|database]]. Susanna verrà riconosciuta tramite il cookie di risposta e si potrà inviarle contenuti personalizzati.
...
Questi tre concetti formano quella che viene spesso definita la triade della CIA. I tre concetti incarnano gli obiettivi di [[3.1 Sicurezza|sicurezza]] fondamentali sia per i dati che per i servizi informatici e informatici. *FIPS PUB 199* fornisce un'utile caratterizzazione di questi tre obiettivi in termini di requisiti e la definizione di una perdita di [[3.1 Sicurezza|sicurezza]] in ciascuna categoria:
...
- **Integrità (copre sia i dati che l'[[2.2 Integrità del sistema|integrità del sistema]]):** protezione contro la modifica o la distruzione improprie delle informazioni e include garantire il non ripudio e l'autenticità delle informazioni. Una perdita di integrità è la modifica o distruzione non autorizzata di informazioni;
...
Sebbene l'uso della triade della CIA per definire gli obiettivi di [[3.1 Sicurezza|sicurezza]] sia ben consolidato, alcuni nel campo della [[3.1 Sicurezza|sicurezza]] ritengono che siano necessari concetti aggiuntivi per presentare un quadro completo. Due dei più comunemente citati sono:
...
- **Responsabilità:** l'obiettivo di [[3.1 Sicurezza|sicurezza]] che genera il requisito per le azioni di un'entità da ricondurre in modo univoco a tale entità.
