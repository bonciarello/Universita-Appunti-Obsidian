...
- è possibile apportare modifiche al [[2.4 Database|database]] direttamente utilizzando SQL o stored procedure;
...
**Un'entità che non è coerente con le regole aziendali è un problema di [[3.1 Sicurezza|sicurezza]].** Ciò è particolarmente vero per un'entità appena creata, quindi è importante che il meccanismo per la creazione di oggetti garantisca che le entità siano coerenti durante la creazione. Potrebbe sembrare ovvio, ma a volte viene trattato come un tecnicismo e le conseguenze potrebbero essere disastrose.

*Un'entità che non è coerente con le regole è un problema di [[3.1 Sicurezza|sicurezza]]* e il modo migliore che abbiamo trovato per contrastare questo rischio è insistere sul fatto che ogni oggetto entità dovrebbe essere coerente immediatamente al momento della creazione.

Poiché le entità rappresentano spesso dati archiviati e modificati per un lungo periodo di tempo, le entità vengono spesso salvate in un [[2.4 Database|database]]. Se si dispone di un [[2.4 Database|database]] relazionale insieme a un mappatore relazionale a oggetti (ORM) come JPA o Hibernate, spesso c'è confusione che porta a una progettazione errata e insicura.
...
Se utilizzi tali framework, hai due opzioni per evitare falle di [[3.1 Sicurezza|sicurezza]] a questo riguardo: *separare il [[2.3 Modello di dominio|modello di dominio]] dal modello di persistenza* o *garantire che il framework di persistenza sia protetto dall'esposizione di oggetti incoerenti*.

La prima alternativa è separarsi concettualmente dal modello di persistenza: se lo fai, il tuo modello di persistenza risiede in un pacchetto separato, insieme ad altro codice dell'infrastruttura. Quando carichi i dati dal [[2.4 Database|database]], il framework di persistenza li carica negli oggetti nel pacchetto di persistenza. Successivamente, si costruiscono oggetti di dominio utilizzando tali oggetti prima di consentire agli oggetti di dominio di gestire le chiamate di logica aziendale. In questo modo, sei completamente responsabile di qualsiasi creazione di oggetti di dominio.
