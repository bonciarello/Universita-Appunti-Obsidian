...
## Perchè REST API?
I siti web monolitici dovrebbero rimanere nel passato perchè il *back-end* è basato su modelli di [[2.4 Database|database]], URL e [[3.4 Viste|viste]] e il *front-end* è basato su modelli di [[2.1 HTML|HTML]], CSS e [[3.1 JavaScript|JavaScript]].
...
## REST
**REST** è un [[3.3 Architettura|architettura]] per la creazione di API su HTTP ed è di tipo *stateless* (ogni richiesta dovrebbe essere indipendente dalle richieste precedenti). Si basa sui verbi HTTP (GET, POST, PUT, DELETE, ...). Rappresenta i dati in JSON o XML.
...
- i *template* conterranno tutte le pagine [[2.1 HTML|HTML]] del progetto;
...
## Avvio del progetto
Possiamo avviare tramite il pulsante *Run* il nostro progetto ma se siamo al primo avvio bisogna anche migrare prima il [[2.4 Database|database]] tramite il comando in console:
```bash
./manage.py migrate
```
Questo comando, nel dettaglio, si occupa di creare o aggiornare il [[2.4 Database|database]]. Se invece di avviare tramite il pulsante *Run* vogliamo avviare il progetto tramite comando in console, dobbiamo scrivere:
```bash
./manage.py runserver
```
...
- cartella *migrations*: memorizza i file di migrazione per aggiornare il [[2.4 Database|database]];
- *admin.py*: aggiungi contenuto al sito di amministrazione;
- *apps.py*: configurazione specifica dell'app;
- *models.py*: modello di [[2.4 Database|database]], test e visualizzazioni.
...
Per impostazione predefinita, Django genera un token per ogni utente e lo memorizza nel [[2.4 Database|database]]. Approcci più sofisticati si basano su *JWT* e *OAuth2* e non necessitano di memorizzare nulla.
...
Specifica autorizzazioni diverse per le [[3.4 Viste|viste]]: in questo caso tutti possono leggere i post, ma solo gli utenti autenticati possono modificarli. Limitiamo la modifica agli autori:
...
- *db* è una **fixtures**, un oggetto che possiamo utilizzare nei nostri test, in questo caso per accedere al [[2.4 Database|database]] temporaneo utilizzato dai test;
