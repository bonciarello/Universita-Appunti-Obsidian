...
**I controlli della tastiera e del mouse sono suddivisi in script separati, un sistema di componenti, come quello di [[2.1 Unity3D|Unity3D]], tende ad essere più flessibile quando le funzionalità sono suddivise in diversi componenti più piccoli.** Il codice dovrebbe essere il seguente:
```csharp
using System.Collections;
...
- "Horizontal" e "Vertical" sono astrazioni per le impostazioni di input in [[2.1 Unity3D|Unity3D]]: 
...
### *OnGUI()*
Unity offre diversi modi per gestire l'[[2.1 Interfaccia utente|interfaccia utente]]: un sistema di [[2.1 Interfaccia utente|interfaccia utente]] di base (IMGUI) e un sistema di [[2.1 Interfaccia utente|interfaccia utente]] avanzato (Unity UI).
Useremo l'[[2.1 Interfaccia utente|interfaccia utente]] di base per visualizzare un punto al centro dello schermo. 
