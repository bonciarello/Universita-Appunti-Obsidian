...
---
Con questo laboratorio, penseremo al posizionamento degli elementi dell'[[2.1 Interfaccia utente|interfaccia utente]] utilizzando i punti di ancoraggio e all'aggiunta di interattività all'[[2.1 Interfaccia utente|interfaccia utente]].

## La nostra [[2.1 Interfaccia utente|interfaccia utente]] pianificata
L'oggetto *Canvas* definisce un'area da visualizzare come [[2.1 Interfaccia utente|interfaccia utente]], ma richiede comunque *Sprite* da visualizzare. Se fai riferimento all'[[2.1 Interfaccia utente|interfaccia utente]] pianificata, c'è un'immagine del "nemico" nell'angolo in alto a sinistra, il testo che mostra il punteggio accanto a quello e un pulsante a forma di ingranaggio nell'angolo in alto a destra.
...
## Posizionamento degli elementi dell'[[2.1 Interfaccia utente|interfaccia utente]]

Iniziamo a posizionare i nostri elementi dell'[[2.1 Interfaccia utente|interfaccia utente]]! Crea un'*Image UI*, un *Button* e un *Text*. Quindi scegli una posizione per l'oggetto dell'[[2.1 Interfaccia utente|interfaccia utente]] nella *Canvas*.
...
Tutti gli oggetti dell'[[2.1 Interfaccia utente|interfaccia utente]] hanno un *anchor*, visualizzata nell'editor come una X di destinazione. Un *anchor (ancora)* è un modo flessibile per posizionare gli oggetti nell'[[2.1 Interfaccia utente|interfaccia utente]].
...
Per impostazione predefinita, gli elementi dell'[[2.1 Interfaccia utente|interfaccia utente]] hanno il loro ancoraggio impostato su *Center*, ma vuoi impostare l'ancora su *Top Left* a sinistra per questa immagine e *Top Right* per il pulsante a forma di ingranaggio.

![](img/sezione15/controllingPosition1.png)

Grazie agli *anchor*, gli oggetti dell'[[2.1 Interfaccia utente|interfaccia utente]] rimarranno nei loro angoli mentre la tela cambia dimensione.
...
Prima di poter interagire con l'[[2.1 Interfaccia utente|interfaccia utente]], è necessario disporre di un cursore del mouse. Se ricordi, questo gioco ha regolato le impostazioni del cursore nel metodo *Start()* del codice *RayShooter*.

Queste impostazioni bloccano e nascondono il cursore del mouse, un comportamento che funziona per i controlli in un gioco FPS ma che interferisce con l'[[2.1 Interfaccia utente|interfaccia utente]]. Puoi commentare quelle righe da *RayShooter.cs* in modo da poter fare clic sull'HUD.
...
## UIController
L'interazione dell'[[2.1 Interfaccia utente|interfaccia utente]] è programmata con una serie standard di passaggi uguali per tutti gli elementi dell'[[2.1 Interfaccia utente|interfaccia utente]]:
- crea un oggetto dell'[[2.1 Interfaccia utente|interfaccia utente]] nella scena (il pulsante creato nelle diapositive precedenti);
- scrivere uno script da chiamare quando si utilizza l'[[2.1 Interfaccia utente|interfaccia utente]];
- allega lo script a un oggetto nella scena;
- collegare gli elementi dell'[[2.1 Interfaccia utente|interfaccia utente]] (come i pulsanti) all'oggetto con quello script.
...
1. pannello degli [[2.4 Eventi|eventi]] *OnClick* vicino alla parte inferiore delle *Impostazioni*;
...
## Pulsante *Chiudi*
Scegli *GameObject -> UI -> Pulsante*, posiziona il nuovo pulsante nell'angolo in alto a destra del popup, trascina lo *Sprite* "chiudi" sulla proprietà *Source Image* di questo elemento dell'[[2.1 Interfaccia utente|interfaccia utente]], quindi fai clic su *Set Native Size* per ridimensionare correttamente l'immagine.
...
## Imposta il pannello degli [[2.4 Eventi|eventi]]
Partendo dal campo di input, nelle impostazioni vedrai un pannello *End Edit*; gli [[2.4 Eventi|eventi]] elencati qui vengono attivati quando l'utente finisce di digitare. Aggiungi una voce a questo pannello, trascina il pop-up nello slot dell'oggetto e scegli *OnSubmitName()* nell'elenco delle funzioni: assicurati di selezionare la funzione nella sezione superiore del pannello *End Edit*, *Dynamic String*, e non nella sezione inferiore, *Static Parameters*.
...
- quindi aggiungi al centro del canvas una nuova [[2.1 Interfaccia utente|interfaccia utente]] immagine;
