...
---
Con questo laboratorio, penseremo all'importazione e riproduzione di clip audio per vari effetti sonori e all'utilizzo dell'[[2.1 Interfaccia utente|interfaccia utente]] per gestire le impostazioni audio.
...
Continuando l'**[[3.3 Architettura|architettura]] del codice** stabilita nelle lezioni precedenti, creerai un **_AudioManager_**. Questo modulo audio centrale ti consentirà di **modulare il volume** dell'audio (e/o della musica) nel gioco e persino silenziarlo.
...
Con *AudioManager* impostato, è ora di dargli la funzionalità di controllo del volume. Questi metodi di controllo del volume verranno quindi utilizzati dai **display dell'[[2.1 Interfaccia utente|interfaccia utente]]** per **disattivare gli effetti sonori o regolare il volume**: in particolare, **aggiornerai la finestra pop-up** con un pulsante e uno slider per controllare le impostazioni del volume. Creerai un nuovo pulsante dell'[[2.1 Interfaccia utente|interfaccia utente]] e un nuovo dispositivo di scorrimento dell'[[2.1 Interfaccia utente|interfaccia utente]] (con un'etichetta di testo dell'[[2.1 Interfaccia utente|interfaccia utente]] "volume").
...
```
Per richiamare le funzioni dal pulsante e dal dispositivo di scorrimento, collegare l'oggetto a comparsa agli **[[2.4 Eventi|eventi]] di interazione** in quei controlli.
...
Ora farai un'altra aggiunta ad *AudioManager* per consentire all'[[2.1 Interfaccia utente|interfaccia utente]] di **riprodurre suoni** quando si **fa clic sui pulsanti**: quando gli effetti sonori emessi da oggetti nella scena, era ovvio dove allegare *AudioSource* ma gli effetti sonori dell'[[2.1 Interfaccia utente|interfaccia utente]] non fanno parte di la scena, quindi imposterai un *AudioSource* speciale solo per *AudioManager*.
...
Ora aggiungi l'effetto sonoro dell'[[2.1 Interfaccia utente|interfaccia utente]] allo script pop-up: trascina l'effetto sonoro dell'[[2.1 Interfaccia utente|interfaccia utente]] sullo slot variabile, ho usato il suono *thump* e quando premi il pulsante dell'[[2.1 Interfaccia utente|interfaccia utente]], quell'effetto sonoro viene riprodotto contemporaneamente.
```csharp
    ...
...
## Controlli della musica dell'[[2.1 Interfaccia utente|interfaccia utente]]
Aggiungiamo più pulsanti all'[[2.1 Interfaccia utente|interfaccia utente]] che riprodurranno musica diversa quando premuti: crea tre nuovi pulsanti in modo da ottenere un popup di impostazione come mostrato nella figura seguente (*Level Music* - *Intro Music* - *No Music*).
...
Puoi premere *Play* ora per verificare che la musica non sia più influenzata dal controllo del volume esistente. Ora aggiungiamo un secondo controllo dell'[[2.1 Interfaccia utente|interfaccia utente]] per il volume della musica, iniziamo regolando *SettingsPopup* come mostrato di seguito.
```csharp
    ...
...
Infine, collega questi controlli dell'[[2.1 Interfaccia utente|interfaccia utente]] al codice in *SettingsPopup*, utilizzando il pannello *OnClick*. Le funzioni che devi selezionare sono *OnMusicToggle()* e *OnMusicValue()* dalla sezione *Dynamic Float* del menu. **Premi *Play* e ora il tuo gioco avrà effetti sonori e musica di sottofondo.**
