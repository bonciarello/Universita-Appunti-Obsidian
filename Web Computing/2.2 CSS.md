...
tags: [web-computing]
---
I **CSS** (*Cascading Style Sheets*) servono a separare il **contenuto** ([[2.1 HTML|HTML]]) dalla **presentazione** (stile grafico).
Vantaggi principali:
...
2.  **Interno:** Blocco `<style>` dentro l'`<head>`.
    ...
    ```
3.  **Inline (Sconsigliato):** Direttamente nel tag [[2.1 HTML|HTML]].
    ```html
    <p style="color: red;">Testo rosso</p>
    ```
## Selettori
Servono a indicare *quali* elementi [[2.1 HTML|HTML]] modificare.
...
```
## Box Model
Ogni elemento [[2.1 HTML|HTML]] è considerato una scatola rettangolare composta da 4 strati (dall'interno all'esterno):
