...
JavaScript è un linguaggio interpretato, dinamico e debolmente tipizzato. Sebbene sia nato per i browser (Client-Side), oggi gira ovunque (Server con Node.js, [[2.4 Database|Database]] come CouchDB, App Desktop con Electron).
### Vantaggi Lato Client
*   **Performance:** Riduce il carico sul server eseguendo calcoli sul dispositivo dell'utente.
*   **Interattività:** Risposta immediata agli [[2.4 Eventi|eventi]] (click, input) senza ricaricare la pagina.
### Inserimento nella Pagina
1.  **Esterno (Best Practice):** File separato `.js`.
    ```html
    <script src="script.js"></script>
    ```
2.  **Embedded:** Blocco script nell'[[2.1 HTML|HTML]].
    ```html
    <script>
      console.log("Ciao!");
    </script>
    ```
3.  **Inline (Sconsigliato):** Negli attributi [[2.1 HTML|HTML]].
    ```html
