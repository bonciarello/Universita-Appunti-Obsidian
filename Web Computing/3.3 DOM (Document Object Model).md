...
tags: [web-computing]
---
Il **DOM** (*Document Object Model*) è l'interfaccia di programmazione per i documenti [[2.1 HTML|HTML]] e XML. Rappresenta la pagina web come una struttura ad albero, dove ogni nodo è un oggetto (es. un tag, un attributo, un testo).
Grazie al DOM, [[3.1 JavaScript|JavaScript]] può accedere e modificare dinamicamente contenuto, struttura e stile della pagina.
## Struttura e Nodi
...
2.  **Element Node:** I tag [[2.1 HTML|HTML]] (`<div>`, `<p>`, `<body>`).
...
### Lettura/Scrittura
*   **`innerHTML`**: Legge/Scrive l'[[2.1 HTML|HTML]] interno (processa i tag).
    ```javascript
...
### Attributi Standard
Possiamo leggere/scrivere attributi [[2.1 HTML|HTML]] (`href`, `src`, `id`...).
```javascript
...
Oltre al documento (`document`), [[3.1 JavaScript|JavaScript]] può interagire con il browser tramite l'oggetto globale `window`.
