...
tags: [web-computing]
---
Il pattern **MVC** è l'[[3.3 Architettura|architettura]] standard per separare la logica di business dall'[[2.1 Interfaccia utente|interfaccia utente]] nelle applicazioni web. Dividere un'app in queste tre componenti facilita la manutenibilità e il lavoro di squadra.
## Le Componenti
### Model (Il Dato)
...
3.  **Model:** Esegue il calcolo o interroga il [[2.4 Database|database]] e restituisce un oggetto (es. `Utente`).
4.  **Inoltro:** Il Controller salva l'oggetto `Utente` nel `request scope` e inoltra la richiesta alla JSP.
5.  **View:** La JSP legge l'oggetto dal `request` e genera l'[[2.1 HTML|HTML]] finale.
...
```
## Vantaggi di MVC
*   **Separazione delle responsabilità:** Se voglio cambiare il [[2.4 Database|database]], tocco solo il Model. Se voglio cambiare il look, tocco solo la View.
*   **Supporto a più interfacce:** Posso usare lo stesso Model per servire una pagina [[2.1 HTML|HTML]] (View JSP) o una risposta JSON per un'App Mobile.
*   **Testabilità:** È molto più semplice testare la logica nel Model se non è mescolata ai tag [[2.1 HTML|HTML]].
