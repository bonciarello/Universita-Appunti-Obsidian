...
tags: [web-computing]
---
La **persistenza** è la capacità di un'applicazione di salvare i dati in modo che sopravvivano alla chiusura del programma. Nelle Java Web App, questo avviene solitamente tramite [[2.4 Database|database]] relazionali (RDBMS) e la libreria **JDBC**.
...
## JDBC (Java [[2.4 Database|Database]] Connectivity)
JDBC è l'API standard di Java per interagire con i [[2.4 Database|database]].
...
*   **IdBroker:** Una classe o procedura che garantisce la generazione di ID univoci (es. tramite `SEQUENCE` in PostgreSQL o `AUTO_INCREMENT` in [[4.3 MySQL|MySQL]]).
